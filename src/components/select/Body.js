...
import {submitModel, addAlgoSelection} from "../../actions/modelAction";
...
const Body = ({backDialogOpen, setBackDialogOpen}) => {
    ...
    //     }
    //     if (algo_selected) {
    //         setAlgoValue(algo_selected);
    //     }
    //
    //     if (range_value) {
    //         setValue(range_value);
    //         setSplit(range_value);
    //     }
    //
    // }, [is_auto, algo_selected, range_value]);
    ...
    const handleChecked = (e) => {
        ...
        // dispatch(addAlgoSelection(isAuto === "Manual" ? "Auto" : "Manual"));
    };
    const handleAlgoChange = (e) => {
        setAlgoValue(e.target.value);
        // dispatch(addAlgoSelection(is_auto, e.target.value));
    };
    const onClick = (e) => {
    ...
    const handleChange = (event, newValue) => {
        if (typeof newValue == 'number') {
            setValue(newValue);
            setSplit(newValue);
            // dispatch(addAlgoSelection(is_auto, algo_selected, newValue));
            console.log(split);
        }
    }
        ...
        >
            ...
            >
                ...
                <Box sx={{ width: '25%', minWidth: '211px' }}>
                                    ...
                                    <Box padding="10px" display="flex" flexDirection="column" gap="10px">
                                        ...
                                        <Box style={{display: 'flex', justifyContent: 'space-between'}}>
                                            <Button variant="contained" startIcon={<ArrowBackIos/>}
                                                    onClick={() => setTooltipId(22)}>PREVIOUS</Button>
                                            <Button variant="contained" endIcon={<ArrowForwardIos/>}
                                                    onClick={() => setTooltipId(24)}>NEXT</Button>
                                        </Box>
                                    </Box>
                                }
                                placement="right"
                                ...
                                <Box>
                                    ...
                                    >
                                        <ToggleButton value="Auto"
                                                      style={{width: '50%'}}>Auto</ToggleButton> {/* And adjust the width here */}
                                        <ToggleButton value="Manual"
                                                      style={{width: '50%'}}>Manual</ToggleButton> {/* And here */}
                                    </ToggleButtonGroup>
                                </Box>
                            </CustomTooltip>
                            <TextField
                                ...
                                {model && model.model_type === "RG"
                                    ? regressionAlgos.map((algo) => (
                                        <MenuItem key={algo.value} value={algo.value}>
                                            <HtmlTooltip placement="right" title={algo.tooltip}
                                                         sx={{fontFamily: "'SF Pro Display', sans-serif",}}>
                                                <Box sx={{width: "100%"}}>
                                                    <Typography
                                                        sx={{fontFamily: "'SF Pro Display', sans-serif",}}>{algo.name}</Typography>
                                                </Box>
                                            </HtmlTooltip>
                                        </MenuItem>
                                    ))
                                ...
                                title={
                                    tooltipId === 24 ? (
                                        <Box padding="10px" display="flex" flexDirection="column" gap="10px">
                                            ...
                                            <Box style={{display: 'flex', justifyContent: 'space-between'}}>
                                                <Button variant="contained" startIcon={<ArrowBackIos/>}
                                                        onClick={() => setTooltipId(23)}>PREVIOUS</Button>
                                                <Button variant="contained" endIcon={<ArrowForwardIos/>}
                                                        onClick={() => setTooltipId(25)}>NEXT</Button>
                                            </Box>
                                        </Box>
                                    ) : (
                                        <Box padding="10px" display="flex" flexDirection="column" gap="10px">
                                            ...
                                            <Box style={{display: 'flex', justifyContent: 'space-between'}}>
                                                <Button variant="contained" startIcon={<ArrowBackIos/>}
                                                        onClick={() => setTooltipId(24)}>PREVIOUS</Button>
                                                <Button variant="contained" endIcon={<ArrowForwardIos/>}
                                                        onClick={() => setTooltipId(26)}>NEXT</Button>
                                            </Box>
                                        </Box>
                                    )
                                }
                            ...
                            {model.model_type === "RG" ?
                                ...
                                <>
                                    ...
                                    <CustomTooltip
                                        ...
                                        title={
                                            <Box padding="10px" display="flex" flexDirection="column" gap="10px">
                                                <Typography>{"Label the values that represent 0 and 1 in your prediction column."}
                                                    <br/> {"Example Labels: 0=Not Spam & 1=Spam, 0=Healthy & 1=Diseased "}
                                                </Typography>
                                                <Box style={{display: 'flex', justifyContent: 'space-between'}}>
                                                    <Button variant="contained" startIcon={<ArrowBackIos/>}
                                                            onClick={() => setTooltipId(30)}>PREVIOUS</Button>
                    ...
                    <Box sx={{ width: '25%', minWidth: '230px' }}>
                        ...
                        <CustomTooltip
                            ...
                            title={
                                <Box padding="10px" display="flex" flexDirection="column" gap="10px">
                                    <Typography>Get a quick glance at your data. Click on the columns and review the top
                                        rows of your
                                        dataset to ensure correct column mapping.</Typography>
                                    <Box style={{display: 'flex', justifyContent: 'space-between'}}>
                                        <Button variant="contained" startIcon={<ArrowBackIos/>}
                                                onClick={() => setTooltipId(31)}>PREVIOUS</Button>
                                        <Button variant="contained" endIcon={<ArrowForwardIos/>}
                                                onClick={() => setTooltipId(33)}>NEXT</Button>
                                    </Box>
                                </Box>
                            }
                            placement="left"
                                    ...
                                    <tr> {/* Add this <tr> tag to wrap your <TableCell> tags */}
                                        {alignment.map((one, index) =>
                                            <th key={one}> {/* Use <th> for header cells */}
                                                <Typography fontSize="1.2em" style={{
                                                    width: "fit-content",
                                                    fontFamily: "'SF Pro Display', sans-serif",
                                                    fontWeight: "800",
                                                    padding: "5px"
                ...
                <Box sx={{display: "flex", justifyContent: "flex-end", marginTop: "1em"}}>
                    <CustomTooltip
                        ...
                        title={
                            <Box padding="10px" display="flex" flexDirection="column" gap="10px">
                                ...
                                <Box style={{display: 'flex', justifyContent: 'space-between'}}>
                                    <Button variant="contained" startIcon={<ArrowBackIos/>}
                                            onClick={() => setTooltipId(32)}>PREVIOUS</Button>
                    ...
                    >
                        <Box>
                            <Button variant="contained" disabled={disabled} sx={{borderRadius: "15px"}}
                                    onClick={onClick}>
                                Train Model
                            </Button>
                        </Box>
